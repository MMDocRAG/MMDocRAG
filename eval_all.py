...
model_dict={"Qwen2.5-3B-Inst":"qwen2.5-3b",
...
def calculate_all(gold_data,eval_data,llm_data):
    ...
    bleu_score_list, rouge1_list, rouge2_list, rougel_list = [], [], [], []
    ...
    for gold_item, eval_item in tqdm(zip(gold_data, eval_data)):
        ...
        txt_quotes, img_quotes, eval_quotes = extract_citations(eval_item["response"])
        ...
        predicted_str = eval_item["response"]
        bleu_score = calculate_bleu(gold_str, predicted_str)
        bleu_score_list.append(bleu_score)

        rouge = calculate_rouge(gold_str, predicted_str)
        r1_prec, r1_rec, r1_f1 = rouge["rouge1"].precision, rouge["rouge1"].recall, rouge["rouge1"].fmeasure
        r2_prec, r2_rec, r2_f1 = rouge["rouge2"].precision, rouge["rouge2"].recall, rouge["rouge2"].fmeasure
        rl_prec, rl_rec, rl_f1 = rouge["rougeL"].precision, rouge["rougeL"].recall, rouge["rougeL"].fmeasure

        rouge1_list.append(r1_f1)
        rouge2_list.append(r2_f1)
        rougel_list.append(rl_f1)

    final_precison = sum(prec_list) / len(prec_list)
    final_recall = sum(recall_list) / len(recall_list)
    final_f1 = sum(f1_list) / len(f1_list)
    # print(f"precision: {final_precison:.3f}\t", f"recall: {final_recall:.3f}\t", f"f1: {final_f1:.3f}")

    final_bleu = sum(bleu_score_list) / len(bleu_score_list)
    final_rouge1 = sum(rouge1_list) / len(rouge1_list)
    final_rouge2 = sum(rouge2_list) / len(rouge2_list)
    final_rougel = sum(rougel_list) / len(rougel_list)
    # print(f"bleu: {final_bleu:.3f}\t", f"rouge1: {final_rouge1:.3f}\t", f"rouge2: {final_rouge2:.3f}\t",
    #       f"rougeL: {final_rougel:.3f}\t")

    img_precison, img_recall, img_f1 = get_scores(gold_img_list, pred_img_list)
    # print(f"img precision: {img_precison:.3f}\t", f"img recall: {img_recall:.3f}\t", f"img f1: {img_f1:.3f}")

    txt_precison, txt_recall, txt_f1 = get_scores(gold_txt_list, pred_txt_list)
    # print(f"txt precision: {txt_precison:.3f}\t", f"txt recall: {txt_recall:.3f}\t", f"txt f1: {txt_f1:.3f}")

    # print(f" {img_precison*100:.1f} & {img_recall*100:.1f} & {img_f1*100:.1f} & {txt_precison*100:.1f} & "
    ...
    #     f"{txt_recall * 100:.1f} & {txt_f1 * 100:.1f} &\\cellcolor{{lightgreen}}{final_f1 * 100:.1f} & {final_bleu:.3f} & {final_rougel:.3f}"
    #     f"&{fluency/count:.2f} & {citation/count:.2f} & {coherence/count:.2f} & {logic/count:.2f} & {factuality/count:.2f} &\cellcolor{{lightred}}{average/count:.2f}"
    # )
    ...
    print(f"final_f1：{final_f1 * 100:.1f}  final_bleu:{final_bleu:.3f}  final_rougel:{final_rougel:.3f}")
...
def get_command_information():
    ...
    # 模式一：使用 model + mode 自动构造路径
    if args.model:
        model = args.model
        mode = args.mode
        eval_path = f'response/{model}_{mode}_response_quotes{setting}.jsonl'
        if model not in model_dict.values():
            print('error, not support that model')
            sys.exit(1)
        else:
            if not os.path.exists(eval_path):
                if model not in model_type[mode]:
                    print(f" Error: model exists but does not support `{mode}` mode. ")
                sys.exit(1)


        llm_judge_path = f'response/evaluation/{model}_{mode}_response_quotes{setting}.jsonl_evaluation.jsonl'
        if not os.path.exists(llm_judge_path):
            print(f" Error: Judge file not found: {llm_judge_path}")
            sys.exit(1)
    ...
    # 模式二：用户自定义路径
        ...
        if not os.path.exists(eval_path):
            print(f" Error: Eval path does not exist: {eval_path}")
            sys.exit(1)
        if not os.path.exists(llm_judge_path):
            print(f" Error: Judge path does not exist: {llm_judge_path}")
            sys.exit(1)
        ...
        print(" Error: Must provide either --model or both --path and --path_judge")
        sys.exit(1)


# 示例调用
if __name__ == '__main__':
    eval_path, llm_judge_path, gold_path = get_command_information()
    print("Eval path:", eval_path)
    print("Judge path:", llm_judge_path)
    print("Gold path:", gold_path)
    gold_data=load_jsonl(gold_path)
    eval_data=load_jsonl(eval_path)
    llm_data=load_jsonl(llm_judge_path)
